    ...
    for year in _recent_quarter_dates():
        try:
            ...
            for _, row in df.head(10).iterrows():
                ratio_raw = row.get("占净值比例", row.get("比例", 0))
                holdings.append({
            ...
            logger.debug(f"[{code}] 持仓 date={year} 失败: {e}")
    return []
...
def fetch_fund_data(code: str, report_days: int = 30, market: Optional[MarketContext] = None) -> FundAnalysisData:
    ...
    try:
        ...
        return FundAnalysisData(info=info, history=history, top_holdings=holdings, market=market)
    except Exception as e:
